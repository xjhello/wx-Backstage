...
'''
统一渲染方法,把g(用于记录用户信息)变量包装到render_template中使得g在每个页面通用
'''
...
def getDictFilterField(db_model, select_filed, key_field, id_list):
    ...
    if id_list and len(id_list) > 0:
        query = query.filter(select_filed.in_(id_list))  # 查询list里面所有的id

    list = query.all()  # 所有的food信息列表
    if not list:
        return ret
    for item in list:
        if not hasattr(item, key_field):
            break
        ret[getattr(item, key_field)] = item  # 在list[{'id':..,},..]中的字典中取出id字段，组装成字典
    return ret  # 返回的字典
...
def selectFilterObj(obj, field):
    ret = []
    for item in obj:
        if not hasattr(item, field):  # 判断对象是否包含对应的属性。
            break
        if getattr(item, field) in ret:  # 返回一个对象属性值。
            continue

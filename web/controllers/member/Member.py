...
def index():
    resp_data = {}
    req = request.values
    page = int(req['p']) if ('p' in req and req['p']) else 1  # 分页
    query = Member.query

    if 'mix_kw' in req:  # 查询字段，iLike忽略大小写
        query = query.filter(Member.nickname.ilike("%{0}%".format(req['mix_kw'])))

    if 'status' in req and int(req['status']) > -1:  # 会员状态的搜索
    ...
    resp_data['search_con'] = req  # 搜索的默认范围
    resp_data['status_mapping'] = app.config['STATUS_MAPPING']
    resp_data['current'] = 'index'  # tab
